...
const routes = ref([
  {name: 'Home', to: '/'},
  {name: t("library_page.route_name"), to: '/library'},
])
...
<template>
  <Banner
      ...
      :label="$t('library_page.banner_label')"/>
    ...
    <div class="md:px-10 lg:px-[100px] xl:px-[150px] px-5 py-10 space-y-5">
      <div class="grid xl:grid-cols-4 md:grid-cols-3 gap-5">
        <a-card v-for="document in documentToShow" data-aos="fade-right"
          ...
          <div class="space-y-2">
            <a v-if="document.content_type === 'LINK'" :href="document.content"
               class="font-bold line-clamp-2 text-ellipsis text-base"
               target="_blank">
              {{ locale === 'en' && !!document.titleEn && document.titleEn !== '' ? document.titleEn : document.title }}
            </a>
            <a v-else
               class="font-bold line-clamp-2 text-ellipsis text-base"
               @click.prevent="handleSeeDetail(document)">
            ...
            <div>
              <div>{{ $t("library_page.resource_label") }} <span>{{ document.source }}</span></div>
              <div>{{ $t("library_page.public_label") }} <span>{{ document.release_date }}</span></div>
            </div>
          </div>
        </a-card>
      </div>
      <div v-if="!isAll" class="text-center">
        <a-button class="min-h-[50px] min-w-[150px]" @click.prevent="showMore">{{ $t("see_more_btn") }}</a-button>
      </div>
    </div>
  </a-spin>
</template>
