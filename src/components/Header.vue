...
<template>
    <div class="sticky top-0 left-0 right-0 z-[999]">
        <div class="xl:h-[50px] md:h-[75px] h-[100px] bg-[#0E7AB7] flex xl:flex-row flex-col xl:items-center items-end py-2 justify-between md:px-[20px] xl:px-[25px] px-[10px] text-white">
            <div class="text-[10px] lg:tracking-[.15em] xl:self-start self-center flex justify-center items-center h-full">
            ...
            <div class="flex items-center justify-end gap-5 xl:w-[35%]">
                <a-input v-model:value="searchStore.searchValue"
                         class="xl:w-[50%] md:w-[200px] w-full rounded-[20px] bg-transparent header-search-input border-white"
                ...
                <div v-if="!userInfo.token && !userInfo.fullName"
                     class="flex items-center md:space-x-5 xl:text-sm text-xs">
                    <router-link class="w-[70px]" to="/register">ĐĂNG KÝ</router-link>
                    <router-link class="xl:w-[90px] w-[85px]" to="/login">ĐĂNG NHẬP</router-link>
                </div>
                <a-dropdown v-else trigger="['click']">
                    <a-avatar class="bg-[#d1d5db] flex justify-center items-center hover:cursor-pointer" size="default">
                        <template #icon>
                            <UserOutlined/>
                        </template>
                    </a-avatar>
        ...
        <div class="flex px-5 md:h-[115px] h-[100px] items-center justify-between bg-[#f4fbff]">
            ...
            <ul :class="[open ? 'left-0' : 'left-[-100%]']"
                ...
                <li v-for="link in links"
                    class="m-0 flex xl:px-0 xl:py-8 py-4 px-5 text-[#5E6C92] tracking-[.01em] hover:cursor-pointer hover:border-b-[3px]
                    ...
                    <a-dropdown v-if="link.children" :trigger="['hover']" @click.prevent>
                        <router-link :to="link.to"
                                     class="text-center md:text-base xl:text-lg text-sm font-semibold py-0 xl:whitespace-pre-wrap"
                            ...
                            <a-menu>
                                <a-menu-item v-for="child in link.children" :key="child.key">
                                    <router-link :to="child.to"
                                                 class="md:text-base xl:text-lg text-sm font-semibold !text-[#5E6C92] !tracking-[.01em]"
                                                 target="_blank">
                    ...
                    <router-link v-else
                                 :to="link.to"
                                 class="text-center md:text-base xl:text-lg text-sm font-semibold py-0 xl:whitespace-pre-wrap align-top"
                                 @click.prevent="handleChangeRoute">
                        {{ link.label.toUpperCase() }}
                    </router-link>
                </li>
