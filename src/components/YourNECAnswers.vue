...
const secondCriteriaAnswers = computed(() => {
	return SecondCriteria.flatMap(group => group.questions).map(questionObj => {
		const answerObj = getAnswerObj(questionObj)
		return {
			key: questionObj.key,
			question: questionObj.question,
...
const thirdCriteriaAnswers = computed(() => {
	return ThirdCriteria.flatMap(group => group.questions).map(questionObj => {
		const answerObj = getAnswerObj(questionObj)
		return {
			key: questionObj.key,
			question: questionObj.question,
...
<template>
    <vue3-html2pdf
        ...
        <template v-slot:pdf-content>
            <div class="py-5">
                <div class="font-bold">
                    1. Nhóm tiêu chí về tầm nhìn và chiến lược áp dụng nguyên tắc KTTH của doanh nghiệp
                </div>
                <div v-for="firstCriteriaQuestionObject in firstCriteriaAnswers">
                    <div>
                        <span class="font-semibold">Câu {{ firstCriteriaQuestionObject.key }}</span>:
                ...
                <div v-for="secondCriteriaQuestionObject in secondCriteriaAnswers">
                    <div>
                        <span class="font-semibold">Câu {{ secondCriteriaQuestionObject.key }}</span>:
                ...
                <div v-for="thirdCriteriaQuestionObject in thirdCriteriaAnswers">
                    <div>
                        <span class="font-semibold">Câu {{ thirdCriteriaQuestionObject.key }}</span>: {{
